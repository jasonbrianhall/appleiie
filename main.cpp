...
gboolean cpu_tick(gpointer data) {
    ...
    if (g_cpu->regPC == lastPC) {
        ...
        if (sameCount > 60) {  // About 1 second at 60 FPS
            debugLog << "CPU appears stuck at $" << std::hex << g_cpu->regPC << std::dec << "\n";
            debugLog << "  A=$" << std::hex << (int)g_cpu->regA 
                     << " X=$" << (int)g_cpu->regX 
                     << " Y=$" << (int)g_cpu->regY 
                     << " SP=$" << (int)g_cpu->regSP << std::dec << "\n";
            debugLog.flush();
            sameCount = 0;
        }
    } else {
...
public:
    ...
    bool loadROM(const std::string& filename) {
        ...
        debugLog << "Loaded " << size << " bytes at $" << std::hex << LOAD << std::dec << "\n";
    ...
    bool loadBASIC(const std::string& filename) {
        ...
        debugLog << "Loaded " << size << " bytes of BASIC at $" << std::hex << LOAD << std::dec << "\n";
        debugLog.flush();
